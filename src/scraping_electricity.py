...
class ElectricData:
    ...
    def get_tohokuden(self):
        ...
        for text in ["先月分実績ダウンロード", "今月分実績ダウンロード"]:
            ...
            sleep(5)
            driver.quit()
        filepath = max(glob.glob(f"{ROOTDIR}/data/**02*zip*"), key=lambda f: os.stat(f).st_mtime)
        print(filepath)
        return filepath
    ...
    def load_with_check(
        ...
        rename_cols = self.config["rename_columns"][area_name]
        common_cols = self.config["common_columns"]
        rename_dict = dict(zip(org_cols, rename_cols, strict=True))
        df = df.rename(columns=rename_dict)
        print("original:", org_cols, "current:", df.columns)
        if len(set(common_cols) - set(df.columns)) > 0:
            raise ValueError
        return df
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--date",
        help="取得する月,指定がなければ当月を設定する('yyyy-mm')",
    )
    parser.add_argument(
        "--file",
        default=None,
